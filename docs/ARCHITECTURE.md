...
| `DartPackageIdentityInfo` | Declaration | The name and language version a `dart_package` states, read back through an attribute by every rule building that package |
...
7. **Source rewriting**: `dart_fix` runs the analyzer's automated fixes. A build action computes them hermetically — staging the project exactly as `dart_analyze_test` does, so the two see the same pubspec stub and the same options — and `bazel run` copies the result into the workspace. Only files Bazel records as `is_source` are ever written back, which is what keeps codegen output resolvable to its importers without being editable; `dart fix`'s own generated-file heuristic cannot be relied on, since it matches `*.g.dart` and no other name. Both products are exposed as output groups (`dart_fix_fixes`, `dart_fix_manifest`) so they can be inspected without running a tool that rewrites sources.
